...
const deploy = async()=>{  // The only reason to create the deploy method here is to use the async await syntax as we can't use await outside a method.
    ...
    const result = await new web3.eth.Contract(compiledWarranty.abi) // We are receiving JSON as the ABI in interface property we don't want to pass the json to create a contract that's why we are parsing(dividing) the JSON file into javascript object.
        .deploy({
            data: compiledWarranty.bytecode.object // In data property we are specifing the bytecode which is the actual raw compiled contract.
        })
        .send({    // It's actually the send method that triggers the communication from web3 off to the network.
            ...
            gas: '5000000', // Gas is the maximum about of wei we can send with the request or the minimum. NOT SURE.
        })
